 ...
 *   maintainers: fgnass <fgnass@gmail.com>, timothygu <timothygu99@gmail.com>
    ...
    (function () {
      ...
      var toggleMenu = function toggleMenu() {
        menuBtn.addEventListener('click', function (e) {
          e.preventDefault();
          var mainHeight = mainContent.clientHeight + 40;
...
function barChart() {
  ...
  var width = 840;
  var height = 420;
  var _fill = _d2.default.scale.category20();
  var interval = [padding, width - padding];
  var dataY = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J'];
  var svgGroup = undefined;
  var svgBars = undefined;
  var svgTexts = undefined;
  var data = createRandomArray(dataNum);

  var scaleX = _d2.default.scale.linear().domain([0, 100]).range(interval);
  var scaleY = _d2.default.scale.ordinal().rangeRoundBands([padding, height - padding], 0.01);

  createGraph();

  _d2.default.select('.js-reset').on('click', function () {
    resetData(false);
  });

  function createGraph() {
    var svg = _d2.default.select(stage).append('svg').attr({
      width: width,
      height: height
    });
    svgGroup = svg.append('g').attr({
      transform: 'translate(0, 20)'
    }).selectAll('g').append('g').data(data).enter().append('g');
    svgBars = svgGroup.append('rect').attr({
      'x': padding,
      y: function y(d, i) {
        return i * 37;
      },
    ...
    var svgAxisX = _d2.default.svg.axis().scale(scaleX).orient('bottom');
    svg.append('g').attr({
      'class': 'u-graph__axis',
      'transform': 'translate(0, 385)'
    }).call(svgAxisX);
    var svgAxisY = _d2.default.svg.axis().scale(scaleY).orient('left');
    svg.append('g').attr({
      'class': 'u-graph__axis',
  ...
  function resetData(delay) {
    var newData = createRandomArray(dataNum);
    var beforeData = data;
    data = newData;

    svgBars.data(newData).transition().delay(function (d, i) {
      if (!delay) {
        return 0;
      }
      return i * 200;
    ...
    if (_d2.default.select('.u-graph__text')[0][0]) {
      svgTexts.remove();
    }
    svgTexts = svgGroup.append('text').attr({
      'x': padding,
      y: function y(d, i) {
        return i * 37;
      },
    ...
    }).data(newData).each(function () {
      svgGroup.selectAll('text').transition().tween('text', tweenText(beforeData));
    });
  }

  function tweenText(beforeData) {
    return function (d, i) {
      var interpolate = _d2.default.interpolateRound(beforeData[i][0], d[0]);
      return function (t) {
...
function columnChart() {
  ...
  var graphTextTop = _height - 4;
  var maxY = 100;
  var data = undefined;
  var dataX = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J'];
  var _fill = _d2.default.scale.category20();
  var intervalY = [_height, 0];
  ...
  svg = _d2.default.select(stage).append('svg').attr({
    width: width + margin.left + margin.right,
    height: _height + margin.top + margin.bottom
  });
  scaleX = _d2.default.scale.ordinal().rangeRoundBands([0, width], 0.1);
  scaleY = _d2.default.scale.linear().rangeRound(intervalY);
  createGraph();
  ...
  function createGraph() {
    svgGroup = svg.selectAll('g').data(data).enter().append('g').attr({
      width: width + margin.left + margin.right,
      height: _height + margin.top + margin.bottom,
    ...
    svg.append('g').attr({
      'class': 'u-graph__axis',
      'transform': 'translate(' + margin.left + ', ' + (_height + margin.top) + ')'
    }).call(svgAxisX);
  ...
  function resetData(delay) {
    ...
    svgTexts = svgGroup.append('text').attr({
      ...
      svgGroup.selectAll('.u-graph__text').transition().tween('text', tweenText(beforeData));
    });
  }
...
function lineChart() {
  ...
  function parseDate() {
    var parse = _d2.default.time.format('%Y/%m/%d').parse;
  ...
  function init() {
    scaleX = _d2.default.time.scale().range([0, width]);

    scaleY = _d2.default.scale.linear().range([height, 0]);

    axisX = _d2.default.svg.axis().scale(scaleX).orient('bottom').ticks(8).tickFormat(_d2.default.time.format('%y/%m/%d'));

    axisY = _d2.default.svg.axis().scale(scaleY).orient('left');
    ...
    _d2.default.select('.js-selectBox').on('change', function () {
      resetData(false);
    });
  }

  function createGraph() {
    if (_d2.default.select('.js-line')[0][0]) {
      svg.remove();
    }
    ...
    svgGroup.selectAll('.dots').data(data).enter().append('circle').attr({
      fill: function fill(d, i) {
        return _fill(i);
      },
    ...
    var axisXTop = height + margin.top;
    svg.append('g').attr({
      'class': 'u-graph__axis',
    ...
    // Y軸用 罫線の作成
    _d2.default.select('.axis-y').selectAll('line').attr({
      'x1': '5',
      'x2': width,
      'stroke-width': '0.2',
      'stroke-opacity': '0.4'
  ...
  function resetData(reset) {
    ...
    scaleX.domain(_d2.default.extent(data, function (d) {
      return d.date;
    }));
    scaleY.domain([0, maxY]);

    // 線描画用の関数を定義
    lineDrawing = _d2.default.svg.line().x(function (d) {
      return scaleX(d.date);
    }).y(function (d) {
      return scaleY(d.value);
    });
    var mode = document.chkModeForm.selectMode.value;
    lineDrawing.interpolate('' + mode);

    createGraph();

    var targetElem = document.querySelector('.js-line');
    var targetLength = targetElem.getTotalLength();
  ...
  function strokeAnimation(elem, length, duration, repeat) {
    ...
    elem.style.transition = elem.style.WebkitTransition = 'stroke-dashoffset ' + duration + 's ease-in-out';
    if (repeat) {
...
},{"d3":1}],11:[function(require,module,exports){
'use strict';
...
function pieChart() {
  ...
  var createRandomValue = function createRandomValue(count) {
    var returnArray = [];
    for (var i = 0; i < count; i++) {
      returnArray.push(Math.floor(Math.random() * 100));
    }
  ...
  var resetOrderZ = function resetOrderZ() {
    for (var i = 0; i < dataNum; i++) {
      var targetDOM = _d2.default.select('#g-' + i)[0][0];
      var targetParent = _d2.default.select('#parent-g')[0][0];
      targetParent.appendChild(targetDOM);
    }
  };

  var arcTweenFirst = function arcTweenFirst(a) {
    var start = {
      startAngle: 0,
      endAngle: 0
    };
    var i = _d2.default.interpolate(start, a);
    this._current = i(0);
    return function (t) {
      return arc(i(t));
    };
  ...
  var resetData = function resetData() {
    var data = createRandomValue(dataNum);

    resetOrderZ(); // アニメーションが汚くなるので、svg要素の順番を戻す

    gPie = svgGroup.selectAll('g').data(pie(data)).attr({
    ...
    }).transition().duration(1000).ease('elastic').attrTween('d', arcTween);

    gPie.select('text').attr({
      transform: function transform(d) {
        var c = arc.centroid(d);
        var textScale = 1.3; // 中心からの位置を調整
  ...
  var arcOver = _d2.default.svg.arc().outerRadius(outerR * 1.1).innerRadius(0);

  var drawPie = function drawPie(data, cx, cy) {
    gPie = svgGroup.selectAll('g').data(pie(data)).enter().append('g').attr({
      transform: 'translate(' + cx + ', ' + cy + ')',
      id: function id(d, i) {
        return 'g-' + i;
      }
    }).on('mouseover', function () {
      var targetId = _d2.default.select(this).attr('id').replace(/g-/, '');
      var targetPie = '#pie-' + targetId;
      var targetText = '#text-' + targetId;

      this.parentNode.appendChild(this); // svg要素にz-indexは効かないので、DOMの順序を下に持ってくる

      _d2.default.select(targetPie).transition().duration(200).attr({
        'd': arcOver,
        'fill-opacity': 0.9
      });
      _d2.default.select(targetText).transition().duration(200).attr({
        'fill-opacity': 1
      });

      _d2.default.select(this).sort(function () {
        return 1;
      });
    ...
    }).transition().duration(600).ease('linear').attrTween('d', arcTweenFirst).each(function (d) {
      this._current = d;
    });

    gPie.append('text').attr({
      transform: function transform(d) {
        var c = arc.centroid(d);
        var textScale = 1.3; // 中心からの位置を調整
        return 'translate(' + c[0] * textScale + ', ' + c[1] * textScale + ')';
  ...
  var init = function init() {
    ...
    pie = _d2.default.layout.pie().sort(null).value(function (d) {
      return d;
    });

    arc = _d2.default.svg.arc().outerRadius(outerR).innerRadius(innerR); // innerRadiusを与えるとドーナツ型になる
  ...
  Router.prototype.run = function (currentPathName) {
    ...
    return $.each(this._routes, function (item) {
      return function (pathName, action) {
        var globPath = item._getGlobPath(pathName);
        var regexp = globToRegexp(globPath, {
          extended: true
        });
...
},{"glob-to-regexp":2,"jquery":4}],13:[function(require,module,exports){
'use strict';
...
var _barChart = require('./contents/barChart');

var _barChart2 = _interopRequireDefault(_barChart);

var _columnChart = require('./contents/columnChart');

var _columnChart2 = _interopRequireDefault(_columnChart);

var _lineChart = require('./contents/lineChart');

var _lineChart2 = _interopRequireDefault(_lineChart);

var _pieChart = require('./contents/pieChart');
...
},{"./common":7,"./contents/barChart":8,"./contents/columnChart":9,"./contents/lineChart":10,"./contents/pieChart":11,"./lib/router":12,"./top":14}],14:[function(require,module,exports){
'use strict';
...
function top() {
  ...
  var domB = document.body;
  var target01 = undefined;
  var target02 = undefined;
  var target03 = undefined;
  var target04 = undefined;
  var baseFireTop = 200;

  var Motion = function () {
    function Motion(elem) {
      var fireTop = arguments.length <= 1 || arguments[1] === undefined ? baseFireTop : arguments[1];
      ...
      _gsap2.default.set(this.targetElem, {
        y: this.fireTop + 300
      });
      this.isShow = false;
    }
  ...
  var showSpinner = function showSpinner() {
    ...
    document.querySelector('.js-spinner-wrapper').appendChild(spinner.el);
  };

  var hideSpinner = function hideSpinner() {
    var spinner = document.querySelector('.spinner');
    var spinnerCover = document.querySelector('.js-spinner-wrapper');
    _gsap2.default.set(spinner, {
      opacity: 0,
  ...
  var coverFitting = function coverFitting() {
  ...
  var setPosition = function setPosition() {
    ...
    var introTop = undefined;

    var titleTop = coverHeight / 2 - titleHeight / 2;
  ...
  var titleShowAnimation = function titleShowAnimation() {
    var title = document.querySelector('.js-title');
    _gsap2.default.set(title, {
      opacity: 0,
      y: -100
    });

    setTimeout(function () {
      _gsap2.default.to(title, 1.5, {
        opacity: 1,
        y: 0
      });
    }, 200);
  ...
  var introShow = function introShow() {
    var intro = document.querySelector('.js-intro');
    _gsap2.default.set(intro, {
      opacity: 0,
      y: -20
    });
    setTimeout(function () {
      _gsap2.default.to(intro, 1.5, {
        opacity: 1,
        y: 0,
        repeat: -1,
        repeatDelay: 2
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var elem01 = document.querySelector('.js-motion-01');
    target01 = new Motion(elem01, baseFireTop);
    var elem02 = document.querySelector('.js-motion-02');
    target02 = new Motion(elem02, baseFireTop + 200);
    var elem03 = document.querySelector('.js-motion-03');
    target03 = new Motion(elem03, baseFireTop + 400);
    var elem04 = document.querySelector('.js-motion-04');
  ...
  window.addEventListener('resize', (0, _lodash2.default)(coverFitting, 200));
}

},{"gsap":3,"lodash.debounce":5,"spin.js":6}]},{},[13]);
